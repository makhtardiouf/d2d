...
// Default IPv4
 bool lteUseIpv6 = false;

 
Ptr<Ipv6> GetIpv6(Ptr<Node> n)
{
    if (n == 0)
    {
        NS_LOG_WARN(n << "Given node is null");
        return 0;
    }

    return n->GetObject<Ipv6>();
}

Ptr<Ipv4> GetIpv4(Ptr<Node> n)
{
    if (n == 0)
    {
        NS_LOG_WARN(n << "Given node is null");
    ...
    return n->GetObject<Ipv4>();
}

Ipv4Address GetIpv4Address(Ptr<Node> n)
{
    ...
    if (ip == 0)
    {
        NS_LOG_WARN(n << "Node has no IPv4");
        return 0;
    }
    return ip->GetAddress(1, 0).GetLocal();
}

Ipv6Address GetIpv6Address(Ptr<Node> n)
{
    NS_LOG_FUNCTION_NOARGS();
    if (n == 0)
    {
    ...
    Ptr<Ipv6> ip = n->GetObject<Ipv6>();
    if (ip == NULL)
    {
        NS_LOG_WARN(n << "Node has no IPv6");
        return Ipv6Address::GetZero();
    }
    // To get the Global Unicast addr use ip->GetAddress(1, 1)
    return ip->GetAddress(1, 0).GetAddress();
}
