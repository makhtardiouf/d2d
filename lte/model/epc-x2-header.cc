...
EpcX2HandoverRequestHeader::Serialize (Buffer::Iterator start) const
{
  ...
  uint8_t ipv6Buf[16]; // IPv6 transport layer address
  ...
  for (int j = 0; j < (int) sz; j++)
    {
      ...
      if(ns3::lteUseIpv6) {
        m_erabsToBeSetupList [j].transportLayerAddress6.GetBytes(ipv6Buf);
        i.WriteU8(*ipv6Buf);
      }
  ...
  for (int j = 0; j < sz; j++)
    {
      ...
      if(ns3::lteUseIpv6) {
        i.Read(ipv6Buf, 16);
        erabItem.transportLayerAddress6 = Ipv6Address (ipv6Buf);
      }
