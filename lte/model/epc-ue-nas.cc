...
EpcUeNas::Send (Ptr<Packet> packet)
{
    ...
    switch (m_state)
    {
        case ACTIVE:
        {
            if (m_inD2dMode)
            {
                NS_LOG_DEBUG ("NAS active, and sending in D2D mode");
                Simulator::ScheduleWithContext (m_d2dPeer.nodeId, MilliSeconds (10.0), &EpcUeNas::DoRecvData, m_d2dPeer.nas, packet);
             //   Simulator::ScheduleWithContext (m_d2dPeer.nodeId, MilliSeconds (10.0), &LteUePhy::PhyPduReceived, m_d2dPeer.phy, packet);
