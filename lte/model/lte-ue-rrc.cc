...
LteUeRrc::DoSendData (Ptr<Packet> packet, uint8_t bid)
{
    ...
    if ((drbid != 0))
    {
        ...
        {
            // Backup Drb in case of future need
            m_drbInfoD2D = Copy<LteDataRadioBearerInfo> (it->second);
            //  it->second->Ref ();
        ...
        if (m_inD2dMode)
            m_drbInfoD2D->m_pdcp->GetLtePdcpSapProvider ()->TransmitPdcpSdu (params);
        else
