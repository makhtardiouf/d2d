 ...
 * $Id: epc-tft-classifier.cc, 2016-03-29 16:19 +0900 makhtar $
 */
...
EpcTftClassifier::EpcTftClassifier ()
{
  ...
  // For ICMPv6
  m_defaultTftId = EpcTft::Default ()->GetDefaultTftId ();
}
...
EpcTftClassifier::Classify (Ptr<Packet> p, EpcTft::Direction direction)
{
  ...
  if (ns3::lteUseIpv6)
  {
    NS_LOG_DEBUG ("EpcTftClassifier IPv6");
  ...
  else if (Icmpv6L4Protocol::PROT_NUMBER)
    {
      Icmpv6Header icmp6Head;
      pCopy->RemoveHeader (icmp6Head);

      uint8_t type = icmp6Head.GetType ();
      NS_LOG_DEBUG ("ICMPv6 type: " << (int)type);
      // ICMP6_ND_ROUTER_SOLICITATION,
      // ICMP6_ND_ROUTER_ADVERTISEMENT
    }
  else {
      //@todo Handle other IPv6-related features
      NS_LOG_WARN ("Using default TFT ID for unknown protocol " << protocol);
      if (!ns3::lteUseIpv6)
        //   return m_defaultTftId;
        // else
        return 0; // no match
    }
  ...
  NS_LOG_INFO ("Classifying packet:"
               << " localAddr=" << localAddress << " " << localAddress6
               << " remoteAddr=" << remoteAddress << " " << remoteAddress6
               << " localPort=" << localPort
  ...
  for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++ it)
  {
    ...
    if ((ns3::lteUseIpv6 && tft->Matches (direction, remoteAddress6, localAddress6, remotePort, localPort, tos))
