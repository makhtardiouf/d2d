...
#include <cstring>  // For SetDiscoveryMsg
...
struct DiscoveryMsgElement : public SimpleRefCount<DiscoveryMsgElement>
{
    DiscoveryMsgElement();
    virtual ~DiscoveryMsgElement();
 ...
 * ProSe D2D
 */
//typedef Callback< void, std::list<Ptr<DiscoveryMsgElement> > > LtePhyDiscoveryCallback;
typedef Callback< void, Ptr<Node> > LtePhyDiscoveryCallback;
typedef Callback< void, Ptr<Node> > LtePhyTxDiscoveryCallback;
typedef Callback< void, Ptr<Node> > LtePhyRxDiscoveryCallback;
...
public:
     ...
     * @param c
     */
    void SetLtePhyDiscoveryCallback(LtePhyDiscoveryCallback c);
    void SetLtePhyTxDiscoveryCallback(LtePhyTxDiscoveryCallback c);
    void SetLtePhyRxDiscoveryCallback(LtePhyRxDiscoveryCallback c);
    ...
    void SetD2dMode(bool inD2dMode)
    {
        m_inD2dMode = inD2dMode;
    }
    ...
    {
        u_int32_t nodeId;
        Ptr<LteSpectrumPhy> spectrumPhy;
    } m_d2dPeer;
    
    void SetDiscoveryMsg(std::string msg)
    {
        // TODO push_back in msg list

        strncpy(m_discoveryMsg.payload, msg.c_str(), sizeof (m_discoveryMsg.payload));
    }

    std::string GetDiscoveryMsg()
    {
        return std::string(m_discoveryMsg.payload);
    }
...
private:
    ...
    // D2D
    // UNUSED
    LtePhyDiscoveryCallback m_ltePhyDiscoveryCallback;
    LtePhyTxDiscoveryCallback m_ltePhyTxDiscoveryCallback;
    LtePhyRxDiscoveryCallback m_ltePhyRxDiscoveryCallback;
    DiscoveryMsgElement m_discoveryMsg;
};
