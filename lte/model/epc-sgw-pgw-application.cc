...
Ipv6Address
EpcSgwPgwApplication::UeInfo::GetEnbAddr6 ()
{
  return m_enbAddr6;
}
...
EpcSgwPgwApplication::RecvFromTunDevice (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
{
  ...
  std::clog << "packet addressed to " << ueAddr;

  std::map<Ipv6Address, Ptr<UeInfo> >::iterator it = m_ueInfoByAddrMap6.find (ueAddr);
  if (it == m_ueInfoByAddrMap6.end ())
  {
    if (ueAddr.IsMulticast ()) {
      Icmpv6Header icmp6Head;
      pCopy->RemoveHeader (icmp6Head);
      std::clog << "ICMPv6 type: " << (int)icmp6Head.GetType () << std::endl;
      // Router Ack
    } else
      NS_LOG_WARN ("Discarding packet for unknown UE address " << ueAddr);
    return succeeded;
  }

  Ipv6Address enbAddr = it->second->GetEnbAddr6 ();
  uint32_t teid = it->second->Classify (packet);
  if (teid == 0)
  {
    NS_LOG_WARN ("no matching bearer for this packet");
...
EpcSgwPgwApplication::AddEnb (uint16_t cellId, Ipv6Address enbAddr, Ipv6Address sgwAddr)
{
  NS_LOG_FUNCTION (this << cellId << enbAddr << sgwAddr);
  EnbInfo enbInfo;
  enbInfo.enbAddr6 = enbAddr;
  enbInfo.sgwAddr6 = sgwAddr;
  m_enbInfoByCellId[cellId] = enbInfo;
}
...
void
EpcSgwPgwApplication::SetUeAddress (uint64_t imsi, Ipv4Address ueAddr, Ipv6Address ueAddr6)
{
  NS_LOG_FUNCTION (this << imsi << ueAddr << ueAddr6);
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  NS_ASSERT_MSG (ueit != m_ueInfoByImsiMap.end (), "unknown IMSI " << imsi);

  m_ueInfoByAddrMap[ueAddr] = ueit->second;
  m_ueInfoByAddrMap6[ueAddr6] = ueit->second;
  ueit->second->SetUeAddr (ueAddr);
  ueit->second->SetUeAddr (ueAddr6);
}
...
EpcSgwPgwApplication::DoCreateSessionRequest (EpcS11SapSgw::CreateSessionRequestMessage req)
{
  ...
  if (ns3::lteUseIpv6)
  {
    Ipv6Address enbAddr6 = enbit->second.enbAddr6;
    ueit->second->SetEnbAddr (enbAddr6);
  }
  ...
  for (std::list<EpcS11SapSgw::BearerContextToBeCreated>::iterator bit = req.bearerContextsToBeCreated.begin ();
       ...
       ++ bit)
    {
      ...
      bearerContext.sgwFteid.address6 = enbit->second.sgwAddr6;
