...
LteUeNetDevice::UpdateConfig (void)
{
    ...
    {
        /*
         * NAS and RRC instances are not ready yet, so do nothing for now and
...
{
    // Desactivated due to debugging verbosity - makhtar
    /// NS_LOG_FUNCTION (this);
    return m_rrc;
}
...
LteUeNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
    ...
    {
        NS_LOG_WARN ("Discarding packet for unsupported protocol " << protocolNumber);
        return false;
    }
    
    if (this->m_inD2dMode)
        NS_LOG_DEBUG("In D2D mode. Checking RLC: " << this->GetRrc ()->GetObject<LteRlcUm> ());
