...
X2IfaceInfo::operator= (const X2IfaceInfo& value)
{
  ...
  m_remoteIpAddr6 = value.m_remoteIpAddr6;
...
EpcX2::AddX2Interface (uint16_t localCellId, Ipv4Address localX2Address, uint16_t remoteCellId, Ipv4Address remoteX2Address)
{
  ...
  if(ns3::lteUseIpv6)
    retval = localX2cSocket->Bind (Inet6SocketAddress (m_localX2Address6, m_x2cUdpPort));
  else
  ...
  if(ns3::lteUseIpv6)
    retval = localX2cSocket->Bind (Inet6SocketAddress (m_localX2Address6, m_x2uUdpPort));
  else
  ...
  Ptr<X2IfaceInfo> x2iinfo = Create<X2IfaceInfo> (remoteX2Address, localX2cSocket, localX2uSocket);
  
  if(ns3::lteUseIpv6)
     x2iinfo->SetIpv6(m_remoteX2Address6);
  
  m_x2InterfaceSockets [remoteCellId] = x2iinfo;
...
EpcX2::DoSendHandoverRequest (EpcX2SapProvider::HandoverRequestParams params)
{
  ...
  NS_LOG_LOGIC ("targetIpAddr6 = " << targetIpAddr6);
...
EpcX2::DoSendHandoverRequestAck (EpcX2SapProvider::HandoverRequestAckParams params)
{
  ...
  NS_LOG_LOGIC ("remoteIpAddr6 = " << remoteIpAddr6);
...
EpcX2::DoSendUeContextRelease (EpcX2SapProvider::UeContextReleaseParams params)
{
  ...
  Ipv6Address remoteIpAddr6;
  if(ns3::lteUseIpv6)  // Don't access the pointer unless v6 is activated
      remoteIpAddr6 = m_x2InterfaceSockets [params.targetCellId]->m_remoteIpAddr6;
...
EpcX2::DoSendLoadInformation (EpcX2SapProvider::LoadInformationParams params)
{
  ...
  if(ns3::lteUseIpv6) targetIpAddr6 = socketInfo->m_remoteIpAddr6;
