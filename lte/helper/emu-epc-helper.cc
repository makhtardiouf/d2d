 ...
 * $Id: emu-epc-helper.cc, 2016-03-29 16:19 +0900 makhtar $
 */
...
EmuEpcHelper::DoInitialize ()
{
  ...
  m_ueAddressHelper6.SetBase (Ipv6Address ("2001:7::"), Ipv6Prefix (104));
  ...
  if (ns3::lteUseIpv6)
    retval = sgwPgwS1uSocket->Bind (Inet6SocketAddress (Ipv6Address::GetAny (), m_gtpuUdpPort));
  else
  ...
  if (ns3::lteUseIpv6)
    m_ueAddressHelper6.Assign (tunDeviceContainer);
  ...
  emu.EnablePcap ("EmuEpcSgw", sgwDevice);

  // we use a /8 subnet so the SGW and the eNBs can talk directly to each other
  const char* net = "10.0.0.0";
  const char* base = "0.0.0.1";
  Ipv6Address net6 = Ipv6Address ("2001:a00::");

  m_epcIpv4AddressHelper.SetBase (net, "255.255.255.0", base);
  m_epcIpv6AddressHelper.SetBase (net6, Ipv6Prefix (120), base);
  m_sgwIpIfaces = m_epcIpv4AddressHelper.Assign (sgwDevices);
  if (ns3::lteUseIpv6)
    m_sgwIpv6Ifaces = m_epcIpv6AddressHelper.Assign (sgwDevices);

  base = "0.0.0.101";
  m_epcIpv4AddressHelper.SetBase (net, "255.0.0.0", base);
  m_epcIpv6AddressHelper.SetBase (net6, Ipv6Prefix (104), base);
...
EmuEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  if (ns3::lteUseIpv6)
    {
      Ipv6InterfaceContainer enbIpv6Ifaces = m_epcIpv6AddressHelper.Assign (enbDevices);
      enbAddress6 = enbIpv6Ifaces.GetAddress (0, 0);
      sgwAddress6 = m_sgwIpv6Ifaces.GetAddress (0, 0);
...
EmuEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
  ...
  if (ns3::lteUseIpv6)
    {
      enb1X2Address6 = GetIpv6Address (enb1);
      enb2X2Address6 = GetIpv6Address (enb2);
    }
