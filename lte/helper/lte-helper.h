 ...
 * SetEpcHelper().
 */
class LteHelper : public Object, public PcapHelperForDevice
{
public:
  LteHelper (void);
  ...
  // makhtar 
  std::map<uint32_t, uint8_t> ActivateDedicatedEpsBearer (NetDeviceContainer ueDevices, EpsBearer bearer);
...
private:
    ...
    /**
     * Entry point for generating pcap files tracing packets of the MAC layer 
     * @param prefix File names prefix
     * @param nd Network device
     * @param promiscuous
     * @param explicitFilename
     */
     virtual void EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous = false, bool explicitFilename = false);
  ...
  void SetD2dMode() {m_isD2dCon = true;}
  ...
  /**
   * D2D Switch to control bearer connection for UEs
   */
  bool m_isD2dCon;
