 ...
 */
#include <ns3/lte-common.h> // EutranMeasurementMapping
                                              ...
                                              double rsrp, double sinr)
{
    ...
    if (m_RsrpSinrFirstWrite == true)
    {
        ...
        outFile << "% Time \tCellId \tIMSI \tRNTI \tRSRP \tRSRP(dBm) \tSinr \tSINR(dB)";
        outFile << std::endl;
    }
    else
    {
    ...
    outFile << rnti << " \t";
    outFile << rsrp << " \t";
    // @todo Re-verify the units
    outFile << ns3::EutranMeasurementMapping::QuantizeRsrp(rsrp) << " \t";
    //outFile << (10 * std::log10(rsrp) << "\t";
    outFile << sinr << " \t";
    outFile << (10 * std::log10(sinr)) << std::endl;
    outFile.close();
}
...
PhyStatsCalculator::ReportUeSinr(uint16_t cellId, uint64_t imsi, uint16_t rnti, double sinrLinear)
{
    ...
    if (m_UeSinrFirstWrite == true)
    {
        ...
        outFile << "% Time \tCellId \tIMSI \tRNTI \tSinrLinear \tSINR(dB)";
        outFile << std::endl;
    }
    else
    {
    ...
    outFile << rnti << " \t";
    outFile << sinrLinear << " \t";
    outFile << (10 * std::log10(sinrLinear)) << std::endl;
    outFile.close();
}
