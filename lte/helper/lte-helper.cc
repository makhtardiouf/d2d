 ...
 * 
 * Added pcap hook - Makhtar Diouf 
 * $Id: lte-helper.cc, 2016-03-29 16:19 +0900 makhtar $
 */
...
LteHelper::ActivateDedicatedEpsBearer (NetDeviceContainer ueDevices, EpsBearer bearer)
{
  ...
  uint8_t bearerId = 0;
  uint32_t j = 0;
  std::map<uint32_t, uint8_t> ueBearerIdMap;

  for (NetDeviceContainer::Iterator i = ueDevices.Begin ();
          i != ueDevices.End (); ++ i, ++ j)
  {
    bearerId = ActivateDedicatedEpsBearer (*i, bearer, EpcTft::Default ());
    ueBearerIdMap.insert (std::pair<uint32_t, uint8_t> (j, bearerId));

  }
  return ueBearerIdMap;
}
...
/*
 *  Adapted from point-to-point-helper
 */
void
LteHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  ...
  // the system.  We can only deal with devices of type LteUeNetDevice.
  //
  //int DLT_MAC_LTE = 147;
  ...
  if (device == 0)
  {
    NS_LOG_WARN ("EnablePcapInternal(): Device " << device << " not of type ns3::LteUeNetDevice");
    return;
  }
  ...
  Ptr<LteUeMac> ueMac = device->GetMac ();

  if (ueMac == 0)
  {
    NS_LOG_WARN ("EnablePcapInternal(): LteUeMac of device " << device << " could not be acquired");
    return;
  }
  pcapHelper.HookDefaultSink<LteUeMac> (ueMac, "Sniffer", file);
}
